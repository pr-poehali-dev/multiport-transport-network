...
def handle_telegram(method: str, event: dict, cursor, conn, cors_headers: dict) -> dict:
    ...
    if action == 'config':
        ...
        if method == 'POST':
            ...
            if not user_email or not telegram_id:
                return {
                    'statusCode': 400,
                    'headers': cors_headers,
                    'body': json.dumps({'error': 'user_email and telegram_id are required'}),
                    'isBase64Encoded': False
                }
            ...
            if not config or not config[0]:
                return {
                    'statusCode': 400,
                    'headers': cors_headers,
                    'body': json.dumps({'error': 'Бот не подключён'}),
                    'isBase64Encoded': False
                }
            ...
            try:
                ...
                if not data.get('ok'):
                    return {
                        'statusCode': 400,
                        'headers': cors_headers,
                        'body': json.dumps({'error': 'Telegram ID не найден. Пользователь должен запустить бота командой /start'}),
                        'isBase64Encoded': False
                    }
                ...
                cursor.execute('''
                    INSERT INTO user_telegram_links (user_id, telegram_id, telegram_username, telegram_first_name)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (user_id) DO UPDATE SET
                        ...
                        updated_at = CURRENT_TIMESTAMP
                ''', (user_id, telegram_id, user_info.get('username'), user_info.get('first_name')))
                conn.commit()
                ...
                return {
                    'statusCode': 500,
                    'headers': cors_headers,
                    'body': json.dumps({'error': f'Ошибка привязки: {str(e)}'}),
                    'isBase64Encoded': False
                }
