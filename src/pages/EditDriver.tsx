...
import { updateDriver, Driver } from '@/api/drivers';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function EditDriver({ driver, onBack, onMenuClick }: EditDriverProps) {
  ...
  useEffect(() => {
    ...
    setPhoneExtra(driver.phoneExtra || '');
    ...
    setPassportSeries(driver.passportSeries || '');
    setPassportNumber(driver.passportNumber || '');
    setPassportDate(driver.passportDate || '');
    setPassportIssued(driver.passportIssued || '');
    ...
    setLicenseSeries(driver.licenseSeries || '');
    setLicenseNumber(driver.licenseNumber || '');
    setLicenseDate(driver.licenseDate || '');
    setLicenseIssued(driver.licenseIssued || '');
  }, [driver]);
  ...
  const handleSave = async () => {
    ...
    try {
      const data = await updateDriver(driver.id!, {
      ...
      toast({
        ...
        description: data.message || 'Данные водителя обновлены'
      });
